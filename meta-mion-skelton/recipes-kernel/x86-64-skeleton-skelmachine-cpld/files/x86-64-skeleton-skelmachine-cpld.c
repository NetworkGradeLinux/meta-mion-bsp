/*
 * Copyright (C)  Jostar yang <jostar_yang@skeleton.com.tw>
 *
 * This module supports the skeleton cpld that hold the channel select
 * mechanism for other i2c slave devices, such as SFP.
 * This includes the:
 *	 Accton skelmachine CPLD
 *
 * Based on:
...
static const struct i2c_device_id skelmachine_cpld_id[] = {
    { "asgvlot64_cpld1", skelmachine_cpld1},
    { "asgvlot64_cpld2", skelmachine_cpld2},
    { "asgvlot64_cpld3", skelmachine_cpld3},
    { }
};
MODULE_DEVICE_TABLE(i2c, skelmachine_cpld_id);
...
enum skelmachine_cpld_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
             char *buf);
static int skelmachine_cpld_read_internal(struct i2c_client *client, u8 reg);
static int skelmachine_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *skelmachine_cpld1_attributes[] = {
...
static struct attribute *skelmachine_cpld2_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
    /*xfp*/
...
static struct attribute *skelmachine_cpld3_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
    /*xfp*/
...
static const struct attribute_group skelmachine_cpld1_group = {
	.attrs = skelmachine_cpld1_attributes,
};


static const struct attribute_group skelmachine_cpld2_group = {
	.attrs = skelmachine_cpld2_attributes,
};

static const struct attribute_group skelmachine_cpld3_group = {
	.attrs = skelmachine_cpld3_attributes,
};
             ...
             char *buf)
{
    ...
    struct skelmachine_cpld_data *data = i2c_get_clientdata(client);
    int status = 0;
    ...
    status = skelmachine_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
    ...
    status = skelmachine_cpld_write_internal(client, reg, status);
    if (unlikely(status < 0)) {
        goto exit;
    }
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = skelmachine_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
             ...
             char *buf)
{
    ...
    for(i=0;i<3;i++)
    {
        /* Read current status */
        mutex_lock(&data->update_lock);
        status = skelmachine_cpld_read_internal(client, led_table.reg[i]);
        if (unlikely(status < 0)) {
            goto exit;
        }
			...
			const char *buf, size_t count)
{
    ...
    for(i=0;i<3;i++)
    {
        ...
        if (led_table.val[i]==1)
            ...
            status |= mask;
        status = skelmachine_cpld_write_internal(client, led_table.reg[i], status);
        if (unlikely(status < 0)) {
            goto exit;
        }
        mutex_unlock(&data->update_lock);
...
static ssize_t init_pon_port_link_led(struct i2c_client *client, enum cpld_type  type)
{
    ...
    switch (type)
    {
        case skelmachine_cpld1:
            skelmachine_cpld_write_internal(client, 0x21, 0xff);
            skelmachine_cpld_write_internal(client, 0x22, 0xff);
            skelmachine_cpld_write_internal(client, 0x23, 0xff);
            skelmachine_cpld_write_internal(client, 0x24, 0xff);
            skelmachine_cpld_write_internal(client, 0x25, 0xff);
            skelmachine_cpld_write_internal(client, 0x26, 0xff);
            skelmachine_cpld_write_internal(client, 0x27, 0xff);
            skelmachine_cpld_write_internal(client, 0x28, 0xff);
            skelmachine_cpld_write_internal(client, 0x29, 0xff);
            skelmachine_cpld_write_internal(client, 0x30, 0xff);
            skelmachine_cpld_write_internal(client, 0x31, 0xff);
            skelmachine_cpld_write_internal(client, 0x32, 0xff);
            break;
        case skelmachine_cpld2:
            skelmachine_cpld_write_internal(client, 0x27, 0xff);
            skelmachine_cpld_write_internal(client, 0x28, 0xff);
            skelmachine_cpld_write_internal(client, 0x29, 0xff);
            skelmachine_cpld_write_internal(client, 0x30, 0xff);
            skelmachine_cpld_write_internal(client, 0x31, 0xff);
            skelmachine_cpld_write_internal(client, 0x32, 0xff);
            skelmachine_cpld_write_internal(client, 0x33, 0xff);
            skelmachine_cpld_write_internal(client, 0x34, 0xff);
            skelmachine_cpld_write_internal(client, 0x35, 0xff);
            skelmachine_cpld_write_internal(client, 0x36, 0xff);
            skelmachine_cpld_write_internal(client, 0x37, 0xff);
            skelmachine_cpld_write_internal(client, 0x38, 0xff);
            break;
        case skelmachine_cpld3:
            skelmachine_cpld_write_internal(client, 0x21, 0xff);
            skelmachine_cpld_write_internal(client, 0x22, 0xff);
            skelmachine_cpld_write_internal(client, 0x23, 0xff);
            skelmachine_cpld_write_internal(client, 0x24, 0xff);
...
static void skelmachine_cpld_add_client(struct i2c_client *client)
{
...
static void skelmachine_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int skelmachine_cpld_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct skelmachine_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct skelmachine_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}
    ...
    {    
        case skelmachine_cpld1:
            group = &skelmachine_cpld1_group;
            init_pon_port_link_led(client, skelmachine_cpld1);
            break;            
        case skelmachine_cpld2:
            group = &skelmachine_cpld2_group;
            init_pon_port_link_led(client, skelmachine_cpld2);
            break;
        case skelmachine_cpld3:
            group = &skelmachine_cpld3_group;
            init_pon_port_link_led(client, skelmachine_cpld3);
            break;
    ...
    skelmachine_cpld_add_client(client);
    return 0;
...
static int skelmachine_cpld_remove(struct i2c_client *client)
{
    struct skelmachine_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    skelmachine_cpld_remove_client(client);
...
int skelmachine_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = skelmachine_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(skelmachine_cpld_read);

int skelmachine_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = skelmachine_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(skelmachine_cpld_write);

static struct i2c_driver skelmachine_cpld_driver = {
	.driver		= {
		.name	= "skelmachine_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= skelmachine_cpld_probe,
	.remove		= skelmachine_cpld_remove,
	.id_table	= skelmachine_cpld_id,
};

static int __init skelmachine_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&skelmachine_cpld_driver);
}

static void __exit skelmachine_cpld_exit(void)
{
    i2c_del_driver(&skelmachine_cpld_driver);
}

MODULE_AUTHOR("Jostar Yang <jostar_yang@skeleton.com.tw>");
MODULE_DESCRIPTION("Accton I2C CPLD driver");
MODULE_LICENSE("GPL");

module_init(skelmachine_cpld_init);
module_exit(skelmachine_cpld_exit);
